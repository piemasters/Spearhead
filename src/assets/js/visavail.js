...
function visavailChart() {
    ...
    function chart(selection) {
        selection.each(function drawGraph(dataset) {
            ...
            // Set custom time format for 24 hour clock
                ...
                .on('mouseover', function (d) {
                    ...
                    div.html(function () {
                            ...
                            if (isDateOnlyFormat) {
                                ...
                                if (d[2] > d3.time.second.offset(d[0], 86400)) {
                                    return output + moment(parseDateTime(d[0])).format('l') + ' ' + moment(parseDateTime(d[0])).format('LTS') + ' - ' + moment(parseDateTime(d[2])).format('L') + ' ' + moment(parseDateTime(d[2])).format('LTS');
                                }
                                return output + moment(parseDateTime(d[0])).format('LTS') + ' - ' + moment(parseDateTime(d[2])).format('LTS');
                            }
                        })
                        .style('left', function () {
            ...
            if (emphasizeYearTicks && !(isYearTick.every(function (d) {
                    return d === true;
            ...
            legend.append('text')
                .attr('x', width + margin.right - 400 + 20)
            ...
            legend.append('rect')
                ...
                .attr('height', 15)
                .attr('width', 15)
                .attr('class', 'rect_is_slow');

            legend.append('text')
                .attr('x', width + margin.right - 300 + 20)
                .attr('y', paddingTopHeading + 8.5)
                .text('Warnings')
                .attr('class', 'legend');
